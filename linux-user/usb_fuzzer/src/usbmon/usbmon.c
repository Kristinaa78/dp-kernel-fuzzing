...
void show_help(void)
{
	...
	fprintf(stdout,
	        "program collects and saves USB traffic captured with usbmon in a binary format\n");
	fprintf(stdout,
	        "------------------------------------------------------------------------------\n");
	fprintf(stdout, "options:\n");
	fprintf(stdout, "  --help       displays help message and exits\n");
	fprintf(stdout,
	        "  --bus        specifies the bus to trace (followed by '/dev/usbmon{number}')\n");
...
int main(int argc, char *argv[])
{
	...
	for (;;) {
		...
		for (unsigned int i = 0; i < nflush; i++) {
			...
			if (hdr->len_cap)
				process_packet(hdr, data, devnum);
		}
	}
